...
sap.ui.controller("ztestplanapprov.view.S1", {
...
bindCartListSet: function(){
	...
	var sPathCartListSet = "/GetTestPlanApproveSet";
	var frameworkODataModel = this.getOwnerComponent().getModel();
...
var sPathCartListSet = "/GetTestPlanApproveSet?$filter=Status eq '"+status+"' and  DateFrom  eq "+null+" and DateTo eq "+null+"";
}else if(dateTo == "undefined-undefined-T00:00:00"){
	var sPathCartListSet = "/GetTestPlanApproveSet?$filter=Status eq '"+status+"' and  DateFrom  eq datetime'"
	+dateFrom+"' and DateTo eq datetime'"+dateFrom+"'";	
}
else {
var sPathCartListSet = "/GetTestPlanApproveSet?$filter=Status eq '"+status+"' and  DateFrom  eq datetime'"
+dateFrom+"' and DateTo eq datetime'"+dateTo+"'";
}		
...
displayRequest: function(e){
	...
	selectedData.level  = data.Level;
	selectedData.status = data.Status;
	
	if( selectedData.status == 'CLSD' || selectedData.status == 'SHCL'){
		sap.m.MessageToast.show("Test Plan is closed.");
		return false
	}
	if( selectedData.status == 'EDIT')
	{
		sap.m.MessageToast.show("Test Plan is under modification.");
		return false
	}

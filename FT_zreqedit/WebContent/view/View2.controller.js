...
sap.ui.controller("zreqedit.view.View2", {
	reqNumber : "",
...
//				oParamsSiteListSet.filters = filters;
		...
		createDataModels: function(){
			...
			category = this.getView().byId("selectTestcat").getSelectedKey();
...
/*		onAfterRendering: function(){
        	...
        	for(var i=0;i<len;i++){
        		...
        		if(testcatg == "05"){
        			row.getCells()[28].setEnabled(true);
        		}
        		if(i==0){
...
//		code added by hans on 12-18-2018
		...
		_onRoute : function(e){
			...
			if( this.Status == 'EDIT' || this.Status == 'PACT' )
				this.getView().byId("idEdit").setVisible(true);
			else
...
//			mySelectMenu.bindAggregation("items","/d/results",oItemSelectTemplate); //bind aggregation, item to Select element with the template selected above
		...
		onChangeTestCat: function(e){
			...
			if(cat == "05"){
				ptqty.setEnabled(true);
				ftqty.setEnabled(false);
			}else{
				ptqty.setEnabled(false);
				ftqty.setEnabled(false);
			}
...
//		code added to remove value state error from tyre details table
	...
	//added by Hans on 10/10/2018
		onRemoveVehicleDetail: function(oEvent){
			...
			var ftFitQty = tbl.getRows()[idx].getCells()[33].getValue();
			if(parseInt(ftFitQty) > 0){
					sap.m.MessageBox.alert(
							" "+ftFitQty+" quantity already fitted for this line item.", {
								icon: sap.m.MessageBox.Icon.WARNING,
...
/*			var tbl=that.getView().byId("TyreDetailTable");
			var len= tbl.getRows().length;
			if(tbl.getRows()[0].getCells()[28].getVisible()==true && tbl.getRows()[0].getCells()[29].getVisible()==true){
			...
			if(diaMNo){
				if(isNaN(diaMNo)){
					...
					if(parseInt(diaMNo) > 100){
						oEvent.getSource().setValue("");
						sap.m.MessageBox.alert(
								"% Wear can not be greater than 100", {
									icon: sap.m.MessageBox.Icon.WARNING,
									title: "Error"
								}
							);
					...
					if(data[k].Group == group){
						if(testCategory !== "05"){
							if(parseInt(tyreVal) > parseInt(data[k].NoOfTyres)){
								sap.m.MessageBox.alert(
										"Cannot Exceed The Tyre Values Of Same Group.", {
											icon: sap.m.MessageBox.Icon.WARNING,
											title: "Error"
										}
									);

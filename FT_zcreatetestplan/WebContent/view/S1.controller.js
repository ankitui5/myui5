...
jQuery.sap.require("com.acute.zcreatetestplan.util.formatter");
var that, arr;
var dt = 0;

sap.ui.controller("com.acute.zcreatetestplan.view.S1", {
...
onInit: function() {
		...
		jQuery.sap.includeStyleSheet(jQuery.sap
				.getModulePath(
						"com.acute.zcreatetestplan.css.style",
						".css"));
		
		this.bindApplicationSet();
		this.bindSegmentSet();
...
//	Fitment tab Table 1 data binding
		...
		this.bindFitmentAxleSet();		
		this.bindCompCodeSet();
		this.setTestMethod();
...
/* Check function to prompt user to fill Test Request Number	*/
	
handleIconTabBarSelect: function(oEvent) {
		
	/*	filter = oEvent.getParameters().key;*/
		var tabBar= this.getView().byId("id_IconTabBar_ctp_WL");
		var TestReq = this.getView().byId("idTestReqNoSelect");	

		if(!TestReq.getSelectedKey())
		{
			tabBar.setSelectedKey("A");
			sap.m.MessageBox.show("Please Select a Test Request Number.", {
				title : "Error",
...
onChangeRequestNo: function(oEvent){
		...
		this.getView().byId("addCartBtn").setEnabled(true);
		var reqNo = oEvent.getSource().getSelectedKey();
	    var RevNo = oEvent.getParameters().selectedItem.getAggregation("customData")[0].getValue();
		var oView = this.getView();
		...
		var sPathgetAllDataSet = "/TestRequestSet(ReqGuid='"+ reqNo +"',Revno='"+ RevNo +"')?$expand=RequestHeadtoItemNvg,RequestHeadtoVehicleNvg,RequestHeadtoCallNvg,RequestHeadtoUsageNvg";
		var frameworkODataModel = this.getOwnerComponent().getModel();
		...
		this.getAttachmentDetails(reqNo); 									// Document upload
...
createDataModels: function(){
		...
		this.ReqGuid = data.ReqGuid;
		this.ReqRev  = data.Revno;
//		Test Requirement Tab data binding
		...
		this.getView().getModel("testRequireJModel").setData(data);
...
//		Fitment tab Table 2 data binding
		...
		if(data.RequestHeadtoItemNvg.results){
			fitmentTb2JModel.setData(data.RequestHeadtoItemNvg.results);
			...
			for(var i=0;i<TyreDetailsTableCount;i++){
				fitmentTb2JModel.getData()[i]["FtPlanned"] = 0;
			}
		}
...
//		Discount tab Table data binding
		...
		if(data.RequestHeadtoItemNvg.results){
			discountJModel.setData(data.RequestHeadtoItemNvg.results);
			
			//clear remarks 
			for(var i =0; i<data.RequestHeadtoItemNvg.results.length;i++){
				discountJModel.getData()[i].AdditionalComments = "";
			}
...
//		Tyre Details Table data binding
		...
		tyreDetailJModel.setData(data.RequestHeadtoItemNvg.results[0]);
		var idIpPsiText = this.getView().byId("idIpPsiText");
		var ipPsi = data.RequestHeadtoUsageNvg.IpPsi;
		idIpPsiText.setText(ipPsi);
		
		var idReccRimText = this.getView().byId("idReccRimText");
		var ipRim = data.RequestHeadtoUsageNvg.FitmentRimRecommended;
...
//		Proposed Vehicle Table data binding
		...
		if(data.RequestHeadtoVehicleNvg.results){
			proposedVehicleJModel.setData(data.RequestHeadtoVehicleNvg.results);
...
//		Callback tab table data binding
		...
		if(data.RequestHeadtoCallNvg.results){
			callRequireJModel.setData(data.RequestHeadtoCallNvg.results);
...
getFitmentTable1Object: function(){
		...
		var idFitmentPlanTable= this.getView().byId("idFitmentPlanTable");
		idFitmentPlanTable.getModel("fitmentTb1JModel").setData(this.fitmentTable1);
...
bindTestCategorySet: function() {
		...
		var getAllDataJModel = this.getView().getModel("getAllDataJModel");
		var testCatKey = getAllDataJModel.getData().TestCategory;
		var category = [];
...
setTestTyre: function(category){
		...
		var getAllDataJModel = this.getView().getModel("getAllDataJModel");
		var data = getAllDataJModel.getData().RequestHeadtoItemNvg.results;
...
bindLocationListSet: function(cat){
	    debugger
		var oViewObj = this.getView();
		var sPathStageListSet;
		/*if (cat == 'PT'){*/
			sPathStageListSet = "/F4LocationSet?$filter=Indicator eq 'P'"; 	
		/*}else {
			sPathStageListSet = "/F4LocationSet?$filter=Indicator eq 'E'";
		}*/
		...
		if(qtyNo){
			if(isNaN(qtyNo)){
				...
				oEvent.getSource().setValue(qtyNo);
					if(rowKey !== "PT"){
						this.setQtyValues(qtyNo, path);
					}else if(rowKey == "PT"){
						this.checkPTValues(qtyNo, path);
					}		
			}else{
					if(rowKey !== "PT"){
						this.setQtyValues(qtyNo, path);
...
setQtyValues: function(value , index){
		...
		for(var j=0; j<groupQtyModel.getData().length;j++){
			...
			for(var i=0;i<idFitmentPlanTable.getRows().length;i++){
				...
				if(rows[i].getCells()[4].getSelectedKey() == groupQtyModel.getData()[j].Group && rows[i].getCells()[6].getSelectedKey() == groupQtyModel.getData()[j].Group){
					...
					if(rows[i].getCells()[0]._getSelectedItemText() !== "PT"){
		...
		var totalQty=0;
		this.setFtPlanned(group, totalQty, index);
		
	},

setFtPlanned: function(group, totalQty, path){
		var groupQtyModel =this.getView().getModel("groupQtyModel");
		var idTyreDetailsTable = this.getView().byId("idTyreDetailsTable");
		var data = idTyreDetailsTable.getModel("fitmentTb2JModel").getData();
		var tbl=this.getView().byId("idFitmentPlanTable");
		for(var i = 0; i< data.length;i++){
//			if test tyre group and bm tyre group are same
			data[i].FtPlanned = groupQtyModel.getData()[i].GroupQty;
			if(data[i].Group == group){
				if(data[i].FtPlanned  > data[i].Nonptqty){
...
setBmQtyValues: function(value, index){
		...
		var totalQty=0;
		this.setFtPlannedBM(group, totalQty, index);
		
	},

setFtPlannedBM: function(group, totalQty, path){
		var idTyreDetailsTable = this.getView().byId("idTyreDetailsTable");
		var data = idTyreDetailsTable.getModel("fitmentTb2JModel").getData();
		var groupQtyModel =this.getView().getModel("groupQtyModel");
		var tbl=this.getView().byId("idFitmentPlanTable");
...
onDiscountChange: function(oEvent){
		...
		if(diaMNo){
			if(isNaN(diaMNo)){
				...
				if( parseInt(diaMNo) > 100 || parseInt(diaMNo) < 0 ){
					oEvent.getSource().setValue("");
					sap.m.MessageBox.alert(
							"Discount value can not be greater than 100 Or Less Than 0", {
...
onTestTyreSelect: function(oEvent){
		...
		var testCatg = tbl.getRows()[path].getCells()[0]._getSelectedItemText();
		if(testCatg !== "PT"){
...
onBMTyreSelect: function(oEvent){
		var path = oEvent.getSource().getParent().getBindingContext("fitmentTb1JModel").getPath().split('/')[1];
		var tbl=this.getView().byId("idFitmentPlanTable");
		var val = tbl.getRows()[path].getCells()[7].setValue();
...
checkPTValues: function(qty, path){
		...
		tbl.getRows()[path].getCells()[8].setValue(qty);
		var ptQty= 0;
...
//			 
			 ...
			 fit.TestingAxle=fitData[i].Axle;
			 fit.TestGroup=tableRows[i].getCells()[4].getSelectedKey();
			 if(fitData[i].Qty){
				 fit.TestTyreQty=parseInt(fitData[i].Qty); 
			 }else{
				 fit.TestTyreQty=0;
			 }
			 fit.BmGroup=tableRows[i].getCells()[6].getSelectedKey();
			 if(fitData[i].BMQty){
				 fit.BmTyreQty=parseInt(fitData[i].BMQty); 
			 }else{
				 fit.BmTyreQty=0;
			 }
			 ...
			 if(fitData[i].VehicleQty){
				 fit.VehQty=parseInt(fitData[i].VehicleQty); 
			 }else{
				 fit.VehQty=0;
			 }
			 fit.Segment=fitData[i].Segment;
			 fit.VehicleApplication=fitData[i].Application;
...
//			 vehicle.TestRequestNumber ="";
			 ...
			 disc.Remarks = discountData[i].AdditionalComments;
...
/*	*/		
			that.saveUploadedDocs(oResponse.data.PlanGuid);         	// document upload
/*	*/		
		};
...
onReset: function(){
	sap.m.MessageBox.show(
		...
		onResetAll: function(){
			...
			this.getView().byId("idTestReqNoSelect").setSelectedKey("");
			...
			this.getView().byId("idIpPsiText").setText("");
//			reset Test Requirement Tab data binding
			this.getView().getModel("testRequireJModel").setData([]);
			this.getView().getModel("testRequireJModel").refresh();

			var idFitmentPlanTable= this.getView().byId("idFitmentPlanTable");
			idFitmentPlanTable.getModel("fitmentTb1JModel").setData([]);
			idFitmentPlanTable.getModel("fitmentTb1JModel").refresh();
//			reset Fitment tab Table 2 data binding
			var idTyreDetailsTable = this.getView().byId("idTyreDetailsTable");			
			idTyreDetailsTable.getModel("fitmentTb2JModel").setData([]);
			idTyreDetailsTable.getModel("fitmentTb2JModel").refresh();
			
//			reset Discount tab Table data binding
			var idDiscountDetailsTable = this.getView().byId("idDiscountDetailsTable");				
			idDiscountDetailsTable.getModel("discountJModel").setData([]);
			idDiscountDetailsTable.getModel("discountJModel").refresh();
			
//			Tyre Details Table data binding
			this.getView().getModel("tyreDetailJModel").setData([]);
			this.getView().getModel("tyreDetailJModel").refresh();
//			reset Proposed Vehicle Table data binding
			var idProposedDetailsTable = this.getView().byId("idProposedDetailsTable");			
			idProposedDetailsTable.getModel("proposedVehicleJModel").setData([]);
			idProposedDetailsTable.getModel("proposedVehicleJModel").refresh();
			
//			reset Callback tab table data binding
			var callbackTable = this.getView().byId("idCallbackDetailTable");	
	        callbackTable.getModel("callRequireJModel").setData([]);
	        callbackTable.getModel("callRequireJModel").refresh();
	        this.fitmentTable1=[];
	        this.getFitmentTable1Object();
		},
	    ...
	    onFileDeleted : function(oEvent) {
	      ...
	      getAttachmentDetails: function(selectedGuId){        //document upload
			...
			createDocsPayload: function(planguid){              							// document upload
				...
				var navArr=[];
				var data = that.getView().getModel("attachmentModel").getData();
				var req = that.getView().getModel("oUploadModel").getData();
				
				for(var i=0;i<req.items.length;i++){
					var obj={};
					obj.FileName = req.items[i].FileName;
					obj.DocNo = req.items[i].DocNo;
					obj.UpdateFlag = req.items[i].UpdateFlag;
					obj.MimeType = req.items[i].MimeType;
					navArr.push(obj);
				}

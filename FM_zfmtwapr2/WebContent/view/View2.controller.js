...
jQuery.sap.require("zfmtwapr2.util.Formatter");
...
var DataArticles, that, twkunnr,twname,customer, kunnrname, hubName, EnrolMode, 
			status,hubCode, reqDate,CustNo, remark, srvNo, revNo;
sap.ui.core.mvc.Controller
		.extend(
				"zfmtwapr2.view.View2",
				{
					onInit : function() {
						debugger
...
//						****************added by Hans on 8 Jan 2019 ***************
					...
					_onRoute : function(e){	
						debugger
									...
									if(SelectedData.status=="A"){
										this.getView().byId("Id_bt1").setVisible(true);
										this.getView().byId("Id_bt3").setVisible(false);
									}else if (SelectedData.status=="E"){
										this.getView().byId("Id_bt3").setVisible(true);
										this.getView().byId("Id_bt1").setVisible(false);
									}									
									
									//selectedData.kunnrName
									that.getView().byId("HeaderIdTit").setText(SelectedData.name);
...
//						}
	    ...
	    // var oFilter1=new sap.ui.model.Filter([oFilter, oFilter2], false /*AND*/);
					...
					that.getView().byId("idlastmilo").setVisible(true);
					that.getView().byId("idlastkm").setVisible(true);
					that.getView().byId("idlasttot").setVisible(true);
					that.getView().byId("idesti").setVisible(false);
					that.getView().byId("idprop").setVisible(false);
				    };
					...
					if(oData.RotFlag == 'X')
						that.getView().byId("rot").setVisible(true);
					else
						that.getView().byId("rot").setVisible(false);
...
//		**************** added by Hans on 8 jan 2019 ***************************	
					...
					if(status=="E"){
						var temp = new sap.m.ColumnListItem({
							...
							valueHelpOnly:true, showValueHelp:false,editable:false,visible:"{path:'Zposition',formatter:'zfmtwapr2.util.Formatter.StpEnable1'}"}),
							
							new sap.m.VBox({items:[new sap.m.Select({selectedKey:"{Owner}",enabled:"{path:'Zposition',formatter:'zfmtwapr2.util.Formatter.StpEnable'}",
							...
							new sap.m.Button({text:"",icon:"sap-icon://record",type:"Default"}),
					...
					tab.bindAggregation("items" , "/RegnoToItemNvg/results", temp );
										...
										var temp = new sap.m.ColumnListItem({
											cells : [
											        ...
											        new sap.m.Input({value:"{StnclNumber}",valueHelpRequest:[that.onStncNo,that],
									             	...
									             	new sap.m.Select({selectedKey:"{RemoveOk}",change:[that.onAction,that],items:[new sap.ui.core.Item({text:"OK",key:"O"}),
									             	new sap.ui.core.Item({text:"Remove",key:"R"})],forceSelection:true}),
						...
						// Create Method for final Save
								...
								debugger
									oReadModel.read("/VehicleRegNoSet(RegNo='"+regno+"',Type='J',TruckKunnr='"+twkunnr+"',Kunnr='"+kunnr+"')?$expand=RegnoToItemNvg/VitemToServiceNvg/IservToSubservNvg,RegnoToServiceNvg/VServToSubservNvg", {
									success : fncSuccess,
									error : fncError 
								});
		 ...
		 var oFilter = new sap.ui.model.Filter("Zposition",sap.ui.model.FilterOperator.Contains,sValue);
										...
										confirm : [ this._handleNewPositionClose, this ],
										cancel : [ this._handleNewPositionClose, this ]
									});
							_valueHelpRotationSelectDialog.setModel(jModel);
							_valueHelpRotationSelectDialog.open();
						},
						_handleNewPositionClose : function(oEvent) {
							var oSelectedItem = oEvent.getParameter("selectedItem");
							if (oSelectedItem) {
							...
							if(evt.getSource().getSelectedKey()=="R"){
								...
								if (!that._RemovalReason) {
									that._RemovalReason = sap.ui.xmlfragment(
										"zfmtwapr2.view.RemoveTyreDetails", that);
						...
						onSerSheetVech:function(evt){
							...
							if (!that._SheetHelpDialog) {
										that._SheetHelpDialog = sap.ui.xmlfragment(
											"zfmtwapr2.view.ServiceSheet", that);
								...
								if (that.data.EnrolMode=="M"){
									...
									var temp = new sap.m.ColumnListItem({
										cells : [
										 ...
										 selected:"{path:'ServiceSelect',formatter:'zfmtwapr2.util.Formatter.SelecFlg'}"
										});
								}
								else{
								var temp = new sap.m.ColumnListItem({
...
//								**********added by Hans *****************
							...
							/*	for(var i=0;i<items.length;i++){
									...
									PCost = parseInt(PCost) + parseInt(items[i].getCells()[2].getText());
								}*/
					...
					onNewSerSheetVech:function(evt){
						...
						if (!that._NewVehicleServiceHelpDialog) {
									that._NewVehicleServiceHelpDialog = sap.ui.xmlfragment(
										"zfmtwapr2.view.NewVehicleService", that);
...
//						code added to enable quantity fields for selected items
						...
						if(this.getView().byId("idMulReading").getValue()==""||parseInt(this.getView().byId("idMulReading").getValue())==0){
							...
							if(evt.getSource().getParent().getCells()[4].getValue()!=""){
								...
								if(G1NsdVal > 0)
									...
									if (!this._EntriesHelpDialog) {
										this._EntriesHelpDialog = sap.ui.xmlfragment(
											"zfmtwapr2.view.NewEntries", this);
									}
										this.getView().addDependent(this._EntriesHelpDialog);
										this.dialogFlag = "N";
						...
						/*	if(dataModel.oData.Zposition =="STP"){
								sap.ui.getCore().byId("idKMCVD").setVisible(false);
							} else{
								sap.ui.getCore().byId("idKMCVD").setVisible(true);
							}*/
					    ...
					    if (len == 0)
							this.onServiceButton.setType("Reject");
						else
						...
						onTabelEntrieOk:function(){
							...
							var rnkmcvd  = sap.ui.getCore().byId("idKMCVD").getValue()
							...
							that.EntreItem.getBindingContext().getObject().KmCovered=rnkmcvd;
						...
						OnTableSelect:function(evt){
							...
							if (!this._CutHelpDialog) {
								this._CutHelpDialog = sap.ui.xmlfragment(
									"zfmtwapr2.view.Cut", this);
								this.getView().addDependent(this._CutHelpDialog);
							}
							that.evt=evt.getParameter("listItem").getBindingContext().getObject();
							that.evtItem=evt.getParameter("listItem");
							
							if(status=="E" && that.data.EnrolMode!="M"){
								 evt.getParameter("listItem").getCells()[2].setEnabled(true);
							}
...
//										
			...
			onReview: function(){
					...
					var mode = "P";
					var SaveMode = "R";
					this.onFitmentCreate(mode,SaveMode);
				}, 		
		...
		onReviewSave: function(){       
			debugger	
			   var tabBar= this.getView().byId("idIconTabBarMulti");
				var valid = this.validations();
				if(valid == false)
					return false;	
				
				// check for service
				var valid=this.checkallservice();
				if (valid==false){
					tabBar.setSelectedKey("B");
					return false
				}
			...
			checkallservice:function(){
				...
				if(that.data.RotFlag=="X" && (rot=="" || rot == "N")){
					valid = false;
					sap.m.MessageToast.show("Please select rotation service");
					return valid;
				};
				
				var RegnoToServiceNvg=that.data.RegnoToServiceNvg.results;				
				for(i=0;i<RegnoToServiceNvg.length;i++){
				  if(RegnoToServiceNvg[i].ServiceSelect!="X"){
					valid = false;
					sap.m.MessageToast.show("Please select all Vehicle Level services");
					return valid;    
				  }
				};				
				
				var RegnoToItemNvg=that.data.RegnoToItemNvg.results;				
				for(var i=0;i<RegnoToItemNvg.length;i++){
				  	var VitemToServiceNvg = RegnoToItemNvg[i].VitemToServiceNvg.results;
				  	
				  	for(var j=0;j<VitemToServiceNvg.length;j++){
				  		 if(VitemToServiceNvg[j].ServiceSelect!="X"){
							valid = false;
							sap.m.MessageToast.show("Please select all Tyre Level services");
							return valid;    
						}
				  	}				 
				};
		...
		onCancel : function(){
			...
			sap.ui.define(["sap/m/MessageBox"], function(MessageBox) {
				MessageBox.show(
					"Are you sure want to cancel Job Card ?", {
		...
		onRemarks : function(){
			...
			remark = sap.ui.getCore().byId("idRemarks").getValue();
				if(remark == ""){
					sap.ui.getCore().byId("idRemarks").setValueState("Error");
					return;
				}
			var mode = "X";
            var SaveMode = "S";
		...
		onFitmentCreate: function(mode,SaveMode){
		 ...
		 sap.m.URLHelper.redirect("/sap/opu/odata/sap/ZFLEET_SRV/TruckWheelOutputFormSet(TWKunnr='"+twkunnr+"',RevNo='"+revNo+"',ServiceNo='"+srvNo+"')/$value", true);	 
			...
			validations:function(){
							...
							for(i=0;i<tab.getItems().length;i++){	
								...
								if(tab.getItems()[i].getCells()[6].getType()!="Accept"){
									...
									if (rot=="M" && tab.getItems()[i].getCells()[1].getEnabled()==true 
								   			     && tab.getItems()[i].getCells()[1].getVisible()==true){
										if(tab.getItems()[i].getCells()[1].getValue()==""){
											tab.getItems()[i].getCells()[1].setValueState(sap.ui.core.ValueState.Error);
											sap.m.MessageToast.show("Select Rotation Position for Every Position");
											valid =  false; 
										}
									}
						...
						onTabelEntrieOk1:function(){
							...
							if(parseFloat(rorignsd)<0.1 || rorignsd==""){
							...
							if(parseFloat(rnnsd1) > parseFloat(rorignsd)){
								sap.m.MessageToast.show("G1 NSD cannot be greater than Orig. NSD.");
								return
							}	
							
							if(parseFloat(rnnsd2) > parseFloat(rorignsd)){
								sap.m.MessageToast.show("G2 NSD cannot be greater than Orig. NSD.");
								return
							}
							if( parseFloat(rnnsd3) > parseFloat(rorignsd)){
								sap.m.MessageToast.show("G3 NSD cannot be greater than Orig. NSD.");
								return
							}		
							
							if(parseFloat(rnnsd4) > parseFloat(rorignsd)){
								sap.m.MessageToast.show("G4 NSD cannot be greater than Orig. NSD.");
								return
							}				
						...
						validSuspended : function(oEvent)
						{ 
							debugger
							var val = oEvent.getSource().getValue();
							if(val){
							...
							//call new customer reg fragment								
								...
								debugger
								this._EntriesHelpDialog = sap.ui.xmlfragment(
										"zfmtwapr2.view.addNewCustomer", this);
								this.getView().addDependent(this._EntriesHelpDialog);
								this._EntriesHelpDialog.open();
								}, 
						...
						//start state 
							...
							this._EntriesHelpDialog = sap.ui.xmlfragment(
									"zfmtwapr2.view.addVehicle", this);
							this.getView().addDependent(this._EntriesHelpDialog);
							this._EntriesHelpDialog.open();
							} 
							},
					...
					oCreateModel1.create("/SaveVehicleRegistrationHeadSet", payload, {
						...
						displayRequestbyCNo: function(e){
						 ...
						 onCustomerName:function(){
							...
							this._EntriesHelpDialog = sap.ui.xmlfragment("zfmtwapr2.view.Customer",this);
							this.getView().addDependent(this._entriesHelpDialog);
											 ...
											 {abc[3].setValueState("Error");
											  ...
											  if (!this._NewServiceSheetDialog) {
													this._NewServiceSheetDialog = sap.ui.xmlfragment(
														"zfmtwapr2.view.NewServiceSheet", this);

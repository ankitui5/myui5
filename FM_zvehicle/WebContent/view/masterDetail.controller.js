...
jQuery.sap.require("ZVEHICLE.util.Formatter");

sap.ui.controller("ZVEHICLE.view.masterDetail", {

onInit:function() {
		debugger
		var that = this;
		jQuery.sap.includeStyleSheet(jQuery.sap.getModulePath("ZVEHICLE.css.update",".css"));
		jQuery.sap.includeStyleSheet(jQuery.sap.getModulePath("ZVEHICLE.css.style",".css"));
		...
		var fncSuccess = function(oData, oResponse){
			...
			if(oData.results.length==1){
				...
				if (!that._FleetDialog) {
					that._FleetDialog = sap.ui.xmlfragment("ZVEHICLE.view.Intial", that);
	...
	onVehicleMakeHelp: function(evt) {
		...
		onConfigCodeHelp: function(evt) {
			...
			addNewItem : function()
			{
				...
				this._EntriesHelpDialog = sap.ui.xmlfragment("ZVEHICLE.view.addVehicle", this);

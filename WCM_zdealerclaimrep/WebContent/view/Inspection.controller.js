...
sap.ui.define([
	...
	"zdealerclaimrep/model/formatter"
], function(Controller, formatter) {
	"use strict";
	var that = this;
	return Controller.extend("zdealerclaimrep.view.Inspection", {
		formatter: formatter,
		onInit: function() {
			...
			"Inspection").attachMatched(this._onRoute, this);
		},
		...
		_onRoute : function(e){
			...
			this.onEnter(this.SelectedData.VarInspNo);
	    },
		
		
		onEnter: function(VarInspNo) {
			debugger
			var that = this;
			...
			oReadModel.read("/GetInspDataSet(IInspNo='" + VarInspNo + "',IClaimNo='')", {
				success: fncSuccess,
				error: fncError
			});
		...
		onFragment: function() {
			if (!this._EntriesHelpDialog) {
				this._EntriesHelpDialog = sap.ui.xmlfragment(
					"zdealerclaimrep.view.Entries", this);
